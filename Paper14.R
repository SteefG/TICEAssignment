...
block_sampler <- function(x, n, block_size) {
  ...
  ### block length block_size, and outputs a vector of bootstrap samples of length n
...
GARCH11 <- function(w, a, b, n, starting_sigma2) {
  ...
  for (i in 2:n) { # Creates the volatilities (sigma2) and returns (y) of a GARCH(1,1)
...
estimate_conditional_variances <- function(y, theta){
  ...
  ### theta = (w_hat, a_hat, b_hat) and outputs a vector of estimated conditional variances
  
  sigmaHat2 <- rep(0,length(y)) # Initialises the estimated conditional variance vector
  sigmaHat2[1] <- theta[1]/(1-theta[2]-theta[3])  #Estimated marginal variance
  
  
  for (i in 2:length(y)){ # Estimates the GARCH volatility using the estimated coefficients
...
residuals <- function(y, theta){
  ### Takes as input a series y and estimated coefficients theta = (w_hat, a_hat, b_hat),
  ### and outputs the residuals after fitting a GARCH(1,1)
...
boot_replicates <- function(y, block_size, theta){
  ### Takes as input a series y, block size block_size, and estimated GARCH coefficients
  ### theta = (w_hat, a_hat, b_hat) and outputs a bootstrapped ystar vector 
  ...
  sigmaStar2 <- rep(0, length(y)) # Initialises a vector of volatility bootstrap replicates
  sigmaStar2[1] <- sigmaHat2[1] # Sets the first replicate equal to the first estimate volatility
  
  yStar <- rep(0, length(y)) # Initialises a vector of return bootstrap replicates
  ...
  for (i in 2:length(y)){ 
    # Obtains the bootstrap replicates y* of the returns by using the sampled residuals
...
boot_forecast <- function(y, forecast_length, block_size, theta){
  ...
  ### 2*forecast_length containing the forecasted returns and then the forecasted volatilities
  ...
  estimated_coefficients <- c(coef(garch_fit)[2], coef(garch_fit)[3], coef(garch_fit)[4])
  ...
  # the first entry is the last "observed" bootstrap volatility replicate: sigma^2*_n
  
  sum <- 0
  for (i in 0:(length(y)-2)){
    sum <- sum + (estimated_coefficients[3]^i)*((y[length(y)-i-1]^2) - (estimated_coefficients[1]/(1-estimated_coefficients[2]-estimated_coefficients[3])))
  }
  
  sigmaStar2[1] <- (estimated_coefficients[1]/(1-estimated_coefficients[2]-estimated_coefficients[3])) + estimated_coefficients[2]*sum
  # sigma^2*_n
  ...
  for (k in 2:(forecast_length+1)){ 
    # Computes the bootstrapped returns and volatilities at n+1, n+2, ..., n+forecast_length
    sigmaStar2[k] <- estimated_coefficients[1] + estimated_coefficients[2]*(yStar[k-1]^2) + estimated_coefficients[3]*(sigmaStar2[k-1])
    yStar[k] <- eStar[k-1]*sqrt(sigmaStar2[k])
  }
  
  return(c(yStar[2:(forecast_length+1)], sigmaStar2[2:(forecast_length+1)])) 
  # We remove the first entry as it's the forecast at horizon 0 (last observation)
}
...
make_forecast <- function(y, forecast_length, B, block_size, theta){
  ...
  ### bootstrap, and the vector of estimated coefficients theta = (w_hat, a_hat, b_hat),
  ### and returns a matrices containing B bootstrap forecasts (from k=1,..., forecast_length) 
  ...
  for (b in 1:B){
    ...
    forecast_sigma[b,] <- boot_forecast(seriesStar, forecast_length, block_size, theta)[(forecast_length+1):(2*forecast_length)]
    forecast_y[b,] <- boot_forecast(seriesStar, forecast_length, block_size, theta)[1:forecast_length]
...
get_asymptotic_CI <- function(y, alpha, theta, forecast_length, sigma2){
  ### Returns the asymptotic confidence interval for a GARCH(1,1) forecasted return
  ### at horizon forecast_length. The method used comes from the paper.
  ...
  expect_sigma2T1 <- (w/(1-a-b)) + ((a + b)^(forecast_length) * (w+a*y[n]^2+b*sigma2[n]))
...
monte_carlo_simulation <- function(n, nr.sim, forecast_length, block_size, B, alpha, R){
  ### Runs a Monte Carlo simulation for the bootstrap CI for forecasted returns and 
  ### volatilities of a GARCH(1,1), and an asymptotic CI for the return.
  ### The simulations are run nr.sim times, and the function returns the average 
  ### coverage and its standard deviation, the percentage of forecasts falling above
  ### and bellow the CI, and the average length of the CI and its standard deviation
  ...
  for (i in 1:nr.sim){
    ...
    for (r in 1:R){
      # Generates R futures values (at horizon forecast_length) of returns and volatilities
      sim_horizon <- GARCH11(w = 0.05, a = 0.1, b = 0.85, (forecast_length+1), simVolatility[n])
      # The starting volatility value is the last observed volatility (so at horizon 0)
      
      sim_future[r,1] <- sim_horizon[(forecast_length+1),1] # Contains R future returns
      sim_future[r,2] <- sim_horizon[(forecast_length+1),2] # Contains R future volatilities
    }
    
    
    # Estimates the GARCH parameters from the simulated data
    garch_specifications <- ugarchspec(variance.model = list(garchOrder = c(1, 1)), mean.model = list(armaOrder = c(0, 0)))
    garch_fit_11 <- ugarchfit(spec = garch_specifications, data = simSeries)
    estimated_coefficients_11 <- c(coef(garch_fit_11)[2], coef(garch_fit_11)[3], coef(garch_fit_11)[4])
    
    
    # Obtains the bootstrap CIs
    boot_CI <- get_CI(simSeries, forecast_length, B, block_size, alpha, estimated_coefficients_11)
    series_CI <- boot_CI[1:2]
    volatility_CI <- boot_CI[3:4]
  
      
    # Obtains the asymptotic CI
    asymptotic_series_CI <- get_asymptotic_CI(simSeries, alpha, estimated_coefficients_11, forecast_length, simVolatility)
    ...
    for (r in 1:R){
      ...
      if (sim_future[r,1] < series_CI[2] && sim_future[r,1] > series_CI[1]) {accept$y[r,1] <- 1}
      else if (sim_future[r,1] > series_CI[2]) {above$y[r,1] <-1 } 
      else if(sim_future[r,1] < series_CI[1]) {below$y[r,1] <-1 }
      
      if (sim_future[r,2] < volatility_CI[2] && sim_future[r,2] > volatility_CI[1]) {accept$sigma2[r] <- 1}
      ...
      if (sim_future[r,1] < asymptotic_series_CI[2] && sim_future[r,1] > asymptotic_series_CI[1]) {accept$y[r,2] <- 1}
    ...
    coverage$y[i,] <- colMeans(accept$y) # Computes the return coverage of this simulation
    # Bootstrap coverage is the first column, asymptotic the second
    coverage$sigma2[i] <- mean(accept$sigma2) # Computes the volatility coverage of this simulation
    
    average_above$y[i,] <- colMeans(above$y) # Computes the proportion of returns 
    # above the CI in this simulation. Bootstrap first column, asymptotic the second
  ...
  boot_coverage_y <- mean(coverage$y[,1]) # Average of the coverages found in each simulation
  boot_sd_coverage_y <- sd(coverage$y[,1]) # and its standard deviation
  
  boot_above_y <- mean(average_above$y[,1]) # Average proportion of futures values 
  # above the CI
  ...
  summary_boot_y <- matrix(c(boot_coverage_y, boot_sd_coverage_y, boot_above_y, boot_below_y, 
                             boot_length_y, boot_sd_length_y), nrow = 1, ncol = 6)
  ...
  summary_boot_y <- matrix(c(boot_coverage_sigma2, boot_sd_coverage_sigma2, 
                             above_sigma2, below_sigma2, 
                             boot_length_sigma2, boot_sd_length_sigma2), nrow = 1, ncol = 6)
  colnames(summary_boot_y) <- c("Average coverage", "SD", "Av. coverage above", 
                                "Av. coverage below", "Average length", "SD")
...
monte_carlo_simulation(500, 5, 50, 20, 199, 0.05, 100)
