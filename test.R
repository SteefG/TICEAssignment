...
boot_forecast <- function(y, forecast_length, block_size, theta, x){
  ### Takes as input a series y (of replicates) from an original series x, 
  ...
  e <- residuals(x, theta) # Residuals from the original series
  ...
  eStar <- block_sampler(e, (forecast_length), block_size) # Sample from the EDF
...
make_forecast <- function(y, forecast_length, B, block_size, theta){
  ...
  for (b in 1:B){
    ...
    forecast_sigma[b,] <- boot_forecast(seriesStar, forecast_length, block_size, 
                                        theta,y)[(forecast_length+1):(2*forecast_length)]
    forecast_y[b,] <- boot_forecast(seriesStar, forecast_length, 
                                    block_size, theta,y)[1:forecast_length]
...
monte_carlo(500, 5, 50, 10, 199, 0.05, 100)
test_3 <- monte_carlo(1000, 25, 50, 1, 199, 0.05, 100)
